...
$I(X;Y) = \mathcal{E}\_{P\_{XY}} log{\frac{P_{XY}}{P_X P_Y}}$ \
...
양변에 기댓값을 씌운다면, \
$I(T;S) \geq log(N) + E\_{q(T,S|C=1)}{log(q(C=1|T,S))}$ \
즉, mutual information의 lower bound가 증가하므로 전체적인 teacher와 student의 의존도도 상승한다고 할 수 있다. 논문에 쓰여진 저자의 말을 빌리면,\
"I(T;S) is the mutual information between the distributions of the teacher and student embeddings. Thus maximizing $E_{q(T,S|C=1)log(q(C=1|T,S)}$ w.r.t the parameters of the student network
...
$\mathcal{L}\_{critic}(h) = \mathcal{E}_{q(T,S|C=1)}\[\log h(T, S)\] + N\mathcal{E}\_{q(T,S | C=0)}\[1 - \log(h(T, S)))\]$\
...
6. 결과적으로 teacher의 output과의 mutual information을 최대화 하는 student를 찾는다. 식으로 나타내면 $f^{S^*} = argmax_{f^S}\mathcal{E}_{q(T,S|C=1)\[\log(h^*(T,S))\]$ 
